#!/bin/sh
# aquawm-wslg: launch aquawm for WSLg using the X11 backend.
#
...
# On real hardware (DRM) or other Wayland/X11 sessions, run ./build/aquawm
...
#
# Usage: ./run-wslg.sh [-- aquawm args...]   (args are passed through)

if [ -z "$DISPLAY" ]; then
    echo "aquawm-wslg: DISPLAY is not set; is WSLg running?" >&2
    exit 1
fi

BIN="$(dirname "$0")/build/aquawm"
if [ ! -x "$BIN" ]; then
    echo "aquawm-wslg: $BIN not found; build first:" >&2
...
tag_outputs() {
    if ! command -v xdotool >/dev/null 2>&1 || ! command -v xwininfo >/dev/null 2>&1; then
        echo "aquawm-wslg: xdotool/xwininfo not found; windows may not appear." >&2
        echo "aquawm-wslg: install them: sudo dnf install xdotool xwininfo" >&2
        return 0
    fi
    n=0
    while [ "$n" -lt 150 ]; do
        ...
        if [ -n "$ids" ]; then
            for id in $ids; do
                xdotool set_window --class aquawm --classname aquawm "$id" 2>/dev/null
    ...
    echo "aquawm-wslg: no output windows found; window may not appear." >&2
}
